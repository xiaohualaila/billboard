...
public class AppPhoneMgr {
    ...
    public boolean isNetWorkEnabled(Context context) {
        if (isNetEnabled(context) || isWifiEnabled(context)) {
            XLog.e("网络已经开启" + isNetEnabled(context) + "    ,    " + isWifiEnabled(context));
            return true;
        } else {
            XLog.e("网络还未开启" + isNetEnabled(context) + "    ,    " + isWifiEnabled(context));
            return false;
        }
    }
    ...
    public boolean isNetworkConnected(Context context) {
        if (isWifiContected(context) || isNetContected(context)) {
            XLog.e("网络连接成功" + isWifiContected(context) + "    ,    " + isNetContected(context));
            return true;
        } else {
            XLog.e("网络连接失败" + isWifiContected(context) + "    ,    " + isNetContected(context));
            return false;
        }
    }
...
//    }
    ...
    public String[] getAppPermissions(PackageInfo packageInfo) throws PackageManager
...
//    }
    ...
    public static void sendMessage(final Context mContext, final String strPhone, final String
            ...
            public void onReceive(Context _context, Intent _intent) {
                switch (getResultCode()) {
                    case Activity.RESULT_OK:
                        ToastManager.showShort(mContext, "短信发送成功");
                        break;
        ...
        mContext.registerReceiver(new BroadcastReceiver() {
            @Override
            public void onReceive(Context _context, Intent _intent) {
                ToastManager.showShort(mContext, "已经成功接收");
            }
     ...
     * @param strMsgContext 短信内容
     */
    public static void toSendMessageActivity(Context mContext, String strPhone, String
            strMsgContext) {
    ...
    public static String getChoosedPhoneNumber(Activity mContext, int resultCode, Intent data) {
        ...
        if (Activity.RESULT_OK == resultCode) {
            ...
            if (phoneNum > 0) {
                ...
                // 获得联系人的电话号码的cursor;
                Cursor phones = mContext.getContentResolver().query(ContactsContract
     ...
     */
    public static void toCameraActivity(Activity mContext, int requestCode, Uri path) {
    ...
    public static void openPDFFile(Context mContext, String filePath) {
        try {
            ...
            ToastManager.showShort(mContext, "未检测到可打开PDF相关软件");
        }
    }
    ...
    public static void openWordFile(Context mContext, String filePath) {
        try {
            ...
            ToastManager.showShort(mContext, "未检测到可打开Word文档相关软件");
        }
    }
    ...
    public static void openOfficeByWPS(Context mContext, String filePath) {
        ...
        try {
            ...
            File file = new File(filePath);
            if (!file.exists()) {
                ToastManager.showShort(mContext, "文件路径不存在");
                return;
            }
            ...
            ToastManager.showShort(mContext, "本地未安装WPS");
        } catch (Exception e) {
            ToastManager.showShort(mContext, "打开文档失败");
        }
    }
    ...
    public static String getAndroidOsSystemProperties(String key) {
        String ret;
        try {
            Method systemProperties_get = Class.forName("android.os.SystemProperties").getMethod
                    ("get", String.class);
